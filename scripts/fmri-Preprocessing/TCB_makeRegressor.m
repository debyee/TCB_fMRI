function TCB_makeRegressor(design_foldername,root_path)
% Takes in design_foldername, which is a GLM run.
...
% design_foldername='design_AllIntervals_rwls';
...
if exist('design_foldername')
    ...
    allSubs = dir(fullfile(root_path,'sub*'));
    ...
    for subi = 1:numSubs
        ...
        %% ------ Create Regressor for Intercepts and Demeaned Blocks ----------
        ...
        allRuns = dir(fullfile(root_path,char(allSubs(subi)),'func/sub*.nii'));
        if size(allRuns,1)>0
            ...
            for runi = 1:numBlocks
                ...
                block_info = niftiinfo(fullfile(root_path,char(allSubs(subi)),'func/',char(allRuns(runi))));
            ...
            if ~exist(design_foldername)
                ...
                designPath = strcat(root_path,char(allSubs(subi)),'/',design_foldername);
                mkdir(designPath)
            end
            %curSubPath = fullfile(cur_path(1:end-7),'spm-data/',char(allSubs(subi)),design_foldername,'curSubRegMat.mat');
            curSubPath = fullfile(root_path,char(allSubs(subi)),design_foldername,'curSubRegMat.mat');
            ...
            for confi = 1:numConfFiles
                ...
                tmpcf = tdfread(fullfile(root_path,char(allSubs(subi)),'confounds/',char(confoundFiles(confi))));
            ...
            curSubPath = fullfile(root_path,char(allSubs(subi)),design_foldername,'curMotionRegMat.mat');
            save(curSubPath,'R','names');
        ...
        disp('root_path not detected. Check that the inputs to matlab function is correct. Regressors could not be created.');
    end
else
