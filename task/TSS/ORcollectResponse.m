function [whichKey, rt] = ORcollectResponse(responseWait,deviceNum,keyArray)
...
while ((GetSecs < responseExit) && isnan(whichKey(1)))
	...
	if ~keyIsDown
        ...
        if dGiven
            ...
            % Modified to force a proper response
            if iscell(whichKey) || ((exist('keyArray','var') && isempty(find(keyArray == whichKey(1)))) || strcmp('space',KbName(keyCode)))
                keyIsDown = 0;
                whichKey = nan;
            end
