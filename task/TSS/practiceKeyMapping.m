function [p,practice,results] = practiceKeyMapping(p,practice,results)
%PRACTICE Summary of this function goes here
%   Detailed explanation goes here

stimuli = p.practiceStimuli.keyMapping;
p.pracWithDeadline = 0;

for trialNum = 1:p.numPracTrials.keyMapping
    
    %%% Add deadline half way through practice (with instructions)
    if trialNum == (p.numPracTrials.keyMapping/2 + 1)
    ...
    practice.keyMapping.stimuli{trialNum} = stim;
    ...
    practice.keyMapping.resp(trialNum) = resp;
    practice.keyMapping.rt(trialNum) = rt;
    practice.keyMapping.acc(trialNum) = acc;
