% Inputs to setMRrespBox:
expParams.MRresp.targetProd = 'Mag Design fMRI joystick';    
expParams.MRresp.buttonArray = [1 4]; % Looking for these buttons

% setMRrespBox sets these variables (which you should otherwise setup as defaults):        
expParams.MRresp.isJoystick = 0;
expParams.MRresp.deviceNum = -1;

% Using setMRrespBox:
[p,expParams] = setMRrespBox(p,expParams);

% Adjusted call to collect response (just added joystick input):
if expParams.MRresp.isJoystick
    [tmpRate,tmpRT] = BAScollectResponse(expParams.CHOICE.maxRT,expParams.deviceNum,expParams.MRresp.buttonArray,expParams.MRresp.isJoystick);
else
    [tmpRate,tmpRT] = BAScollectResponse(expParams.CHOICE.maxRT,expParams.deviceNum,expParams.CHOICE.UKrateKeys,expParams.MRresp.isJoystick);
end

% Main line(s) from collect response that was changed (aside from adding joystick input):            
if ~exist('isJoystick','var')
   isJoystick = 0; 
end
if isJoystick
...
function [whichKey, rt] = BAScollectResponse(responseWait,deviceNum,keyArray,isJoystick)
...
%             whichKey = KbName(keyCode);            
            ...
            if (exist('keyArray','var') && isempty(find(keyArray == whichKey(1))))
                keyIsDown = 0;
                whichKey = NaN;
                rt = NaN;
            end
...
if exist('keyArray','var')
    whichKey = find(keyArray == whichKey(1));
    if isempty(whichKey)
        whichKey = -1;
    end
