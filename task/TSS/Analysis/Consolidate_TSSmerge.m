...
% Set which datasets to consolidate into data files
consolidate_FXC_OA_v510 = 1;
consolidate_TCB_v201 = 1;
consolidate_RewardPenalty_v106 = 1;

% 0 = Don't run the GLMS, 1 = run the GLMS
runGLMs = 0;

% chdir('~/Dropbox (Brown)/ShenhavLab/Experiments/TSS/Analysis');
% isOAsubject = 1

%% LOAD IN ALL OF THE DATA FILES FOR THE TCB fMRI STUDY
if consolidate_TCB_v201
    
    files = dir('../TCB/ResultsScanning/TSS_TCB_2*mat');
    fileNames = {files(:).name};
    
    % skip subjects by ID 2001_02-26-20, 2002_02-23-20
    subToSkip = {'2001_02-26-20','2002_02-23-20'};
    numSubToUse = length(fileNames) - length(subToSkip);

    % iterate over subjects to save data into struct
    ix_numSubToUse = 1;
    for x = 1:length(fileNames)
        file = ['../TCB/ResultsScanning/', fileNames{x}];
        tmp = load(file);
        
        if sum(strcmp(subToSkip,extractBetween(fileNames{x},"TSS_TCB_",".mat")))==0
            data.raw(ix_numSubToUse).p = tmp.p;
            %data.raw(x).p = tmp.practice;
            data.raw(ix_numSubToUse).results = tmp.results;
            ix_numSubToUse=ix_numSubToUse+1;
        end
...
for x = 1:numSubToUse
    disp(x)
        ...
        numTrialsPerInterval = data.raw(x).results.interval.numTrialsResponded;
        numTrialsTotal = sum(data.raw(x).results.interval.numTrialsResponded);
        
        % Interval level properties
        rewLvl = data.raw(x).p.interval.rewardLvl;
        gainLvl = data.raw(x).p.interval.gainLvl;
        penaltyLvl = data.raw(x).p.interval.penaltyLvl;
        
        % Subject ID and version
        subID = str2double(data.raw(x).p.subID);
        ...
        curAcc2 = curCorrectResp==curResp;
        ...
        curPenaltyLvl = ones(1,numTrialsTotal)*(-10);
        ...
        %curIntBasedNumResponded = ones(1,numIntervalsTotal)*(-10);
        %curIntBasedNumAccTrials = ones(1,numIntervalsTotal)*(-10);
        %curIntBasedNumErrorTrials = ones(1,numIntervalsTotal)*(-10);
        %curIntBasedNumPenalizedTrials = ones(1,numIntervalsTotal)*(-10);
        ...
        for intNum = 1:numIntervalsTotal 
            ...
            curIntLength(ix_start:ix_end) = data.raw(x).p.interval.length(intNum);
            ...
            curPenaltyLvl(ix_start:ix_end) = penaltyLvl(intNum);
        ...
        curSubData = table(curSubID',curVersion',curStimWord',curStimInkColor',curBlockType',curBlockNum',...
            curIntNum',curIntLength',curTrialsCompleted',curTrialNuminInt',curRewLvl', curGainLvl',curPenaltyLvl',curStimCongruency',...
        ...
        curSubData_header = {'SubID','Vers','StimWord','StimColor','BlockType','BlockNum',...
            'IntervalNum','IntervalLength','TrialsInInterval','IntervalTrialNum','RewLvl','GainLvl','PenaltyLvl','Congruency',...
...
pathToSave = strcat(pwd,'/TCB/DataFormatted/data_TSS-TCB-fMRIstudy_version201.csv');
