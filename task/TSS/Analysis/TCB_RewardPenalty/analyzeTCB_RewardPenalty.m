...
files = dir('../../TCB_RewardPenalty/Results/TSS_TCB_RewardPenalty_2*mat');
fileNames = {files(:).name};

for x = 1:length(fileNames)
     file = ['../../TCB_RewardPenalty/Results/', fileNames{x}];
     tmp = load(file);
     data.raw(x).p = tmp.p;
     data.raw(x).results= tmp.results;
end
...
for x = 1:length(fileNames)
    %try
        ...
        numBlocks = data.raw(x).p.numBlocks;
        numIntervals = data.raw(x).p.numIntervals;
        numIntervalsPerBlock = data.raw(x).p.numIntervalsPerBlock;
        numIntervalsTotal = length(data.raw(x).results.interval.numTrialsResponded);
        numTrialsTotal = length(data.raw(x).results.trial.resp); % note this might be problematic if not completed data
        
        % Interval level properties
        curIntRewLvl = data.raw(x).results.interval.rewLevel;
        curIntPenLvl = data.raw(x).results.interval.penaltyLevel;
        curIntTotal = 1:numIntervalsTotal;
        curIntLength = data.raw(x).p.interval.length;
        curIntNumErrorTrials = data.raw(x).results.interval.numErrorTrials;
        ...
        % Stimulus properties
        if isempty(data.raw(x).p.stimuli) % s2030 is missing stimulus data
            StimWord = repmat({'NA'},1,numTrialsTotal); 
            StimInkColor = repmat({'NA'},1,numTrialsTotal);
            StimCongruency = repmat({'NA'},1,numTrialsTotal);
        else
        ...
        if isempty(data.raw(x).p.stimuli(:))
            correctResp = NaN(1,numTrialsTotal);
        else
            correctResp = [data.raw(x).p.stimuli(:).ColorAns];
        end
        curCorrectResp = correctResp(1:numTrialsTotal);
        trialsCompletedPerInterval = data.raw(x).results.interval.numTrialsResponded;
        trialsSeenPerInterval = data.raw(x).results.interval.numTrialsSeen;

        % Extract participant responses
        %curInitiationTime = data.raw(x).results.initiationTime;  %OA_param
        curResp = data.raw(x).results.trial.resp;
        curRT = data.raw(x).results.trial.responseTime;
        curTooFast = data.raw(x).results.trial.tooFast;
        curAcc = data.raw(x).results.trial.acc;
        
    %% CREATE ARRAYS FOR ALL TRIAL-BASED VARIABLES

        % Extract Trial results
        curIntNum = data.raw(x).results.trial.intervalNum;
        curTrialNum = data.raw(x).results.trial.overallTrialNum;
        curIntervalTrialNum = data.raw(x).results.trial.intervalTrialNum;
        curRewardLevel = data.raw(x).results.trial.rewardLevel;
        curPenaltyLevel = data.raw(x).results.trial.penaltyLevel;
        
        % Block Number
        curBlockNum = nan(1,numTrialsTotal);
        curBlockType = cell(1,numTrialsTotal);
        blockInt = 1:numIntervalsPerBlock:numIntervals;
        for block = 1:numBlocks
           curBlockNum(curIntNum>=blockInt(block) & curIntNum<(blockInt(block)+numIntervalsPerBlock)) = block ; 
           curBlockType(curIntNum>=blockInt(block) & curIntNum<(blockInt(block)+numIntervalsPerBlock)) = {data.raw(x).p.blockOrder{block}};
        end
        
        % Initialize arrays to fill based on total number of trials
        curIntLength_Trial = nan(1,numTrialsTotal);
        curIntLengthNoISI_Trial = nan(1,numTrialsTotal);
        curTrialsCompletedPerInterval_Trial = nan(1,numTrialsTotal);
        curIntLengthSecs_Trial = nan(1,numTrialsTotal);
        curIntLengthSecsNoISI_Trial = nan(1,numTrialsTotal);
        curIntLengthSecs = nan(1,numIntervalsTotal);
        curIntLengthSecsNoISI = nan(1,numIntervalsTotal);
        curIntCueDisplay = cell(1,numIntervalsTotal);
        curIntCueDisplay_Trial = cell(1,numTrialsTotal);
        curIntTaskBugCorrected = zeros(1,numIntervalsTotal);
        curTaskBugCorrected = zeros(1,numTrialsTotal);
        
        % Iterate over intervals to fill out arrays based on number of trials per interval
        for int = 1:numIntervalsTotal
            
            % Set indices in array to input values
            ix_start = find(isnan(curIntLength_Trial),1);
            ix_end = ix_start + trialsCompletedPerInterval(int) - 1;
            
            % If last trial is no response, then don't include trial.
            if ~isnan(curAcc(ix_end)) 
                removeLast = 0;
            else
                removeLast = 1;
            end
            ...
            curIntLength_Trial(ix_start:ix_end) = curIntLength(int);
            curIntLengthNoISI_Trial(ix_start:ix_end) = curIntLength(int) - ((trialsCompletedPerInterval(int) - removeLast) * data.raw(x).p.timing.isi);
            curTrialsCompletedPerInterval_Trial(ix_start:ix_end) = trialsCompletedPerInterval(int) - removeLast;
            
            % calculate actual timing
            thisIntLengthSecs = data.raw(x).results.timing.intervalEnd(int)-data.raw(x).results.timing.intervalStart(int);
            curIntLengthSecs_Trial(ix_start:ix_end) = thisIntLengthSecs;
            curIntLengthSecsNoISI_Trial(ix_start:ix_end) = thisIntLengthSecs - ((trialsCompletedPerInterval(int) - removeLast) * data.raw(x).p.timing.isi);
            
            % interval variables
            curIntLengthSecs(int) = thisIntLengthSecs;
            curIntLengthSecsNoISI(int) = thisIntLengthSecs - ((trialsCompletedPerInterval(int) - removeLast) * data.raw(x).p.timing.isi);

            % extract cue displayed per trial
            curIntCueDisplay(int) = extractBetween(data.raw(x).p.interval.cueImage{int},'Game/','.png');
            curIntCueDisplay_Trial(ix_start:ix_end) = repmat(extractBetween(data.raw(x).p.interval.cueImage{int},'Game/','.png'),length(ix_start:ix_end),1);
        ...
        % Convert table 2 cell
        %SubData = table2cell(SubData);
        %SubDataInt = table2cell(SubDataInt);
        
        %% COMPILE TRIAL DATA TABLE
        % Create a table of trial data that can be output for analyses (in R)
        % Can also use this table for analyses within matlab
        curSubData = table(curSubID',curVersion',curStimWord',curStimInkColor',curStimCongruency',curBlockType',curIntCueDisplay_Trial',curBlockNum',...
            curIntNum',curTrialNum',curIntLength_Trial',curIntLengthSecs_Trial',curIntLengthNoISI_Trial',curIntLengthSecsNoISI_Trial',curTrialsCompletedPerInterval_Trial',curIntervalTrialNum',curRewardLevel',curPenaltyLevel',...
            curCorrectResp',curResp',curIsComplete',curRT',curAcc',curTaskBugCorrected',curRewardLevel',curPenaltyLevel');
        curSubData_header = {'SubID','Version','StimWord','StimColor','Congruency','BlockType','CueDisplayed','BlockNum',...
            'IntervalNum','TrialNum','IntervalLength_Ideal','IntervalLength_Actual','IntervalLengthNoISI_Ideal','IntervalLengthNoISI_Actual','TrialsCompletedPerInterval','IntervalTrialNum','RewardLevel','PenaltyLevel',...
            'CorrectResp','Resp','IsComplete','RT','Accuracy','TaskBugCorrected','RewardLevelTaskCode','PenaltyLevelTaskCode'};
        curSubData.Properties.VariableNames = curSubData_header;
   

        % CORRECT FOR BUG IN TASKCODE 
        % CueDisplayed not matching reward and penalty level for highrew
        % and lowrew blocks. however edit for all blocks just in case. Note
        % that highrew and lowrew are not labelled corrected in
        % taskcodedebugged subjects...
        
        % highRew: Rew2Pen1 correction (RewardLevel = 1, PenaltyLevel = 0)
        Rew2Pen1_error = find(strcmp(curSubData.CueDisplayed,'Rew2_Pen1') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 1);
        curSubData.PenaltyLevel(Rew2Pen1_error) = 0;
        curSubData.TaskBugCorrected(Rew2Pen1_error) = 1;
%         Rew2Pen1_total = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen1') & curSubData.RewardLevel == 1);
%         Rew2Pen1_correct = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen1') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 0);
%         Rew2Pen1_error = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen1') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 1);
%         if length(Rew2Pen1_total) == length(Rew2Pen1_correct) + length(Rew2Pen1_error)
%             curSubData.PenaltyLevel(Rew2Pen1_error) = 0;
%             curSubData.TaskBugCorrected(Rew2Pen1_error) = 1;
%         else
%             error(['Error! Rew1Pen1 TaskBugCorrected does not match for Subject ', num2str(subID)]);
%         end
            
        % highRew: Rew2Pen2 correction (RewardLevel = 1, PenaltyLevel = 1)
        Rew2Pen2_error = find(strcmp(curSubData.CueDisplayed,'Rew2_Pen2') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 0);
        curSubData.PenaltyLevel(Rew2Pen2_error) = 1;
        curSubData.TaskBugCorrected(Rew2Pen2_error) = 1;
%         Rew2Pen2_total = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen2') & curSubData.RewardLevel == 1);
%         Rew2Pen2_correct = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen2') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 1);
%         Rew2Pen2_error = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew2_Pen2') & curSubData.RewardLevel == 1 & curSubData.PenaltyLevel == 0);
%         if length(Rew2Pen2_total) == length(Rew2Pen2_correct) + length(Rew2Pen2_error)
%            curSubData.PenaltyLevel(Rew2Pen2_error) = 1;
%            curSubData.TaskBugCorrected(Rew2Pen2_error) = 1;
%         else
%             error(['Error! Rew2Pen2 TaskBugCorrected does not match for Subject ', num2str(subID)]);
%         end
       
        % lowRew: Rew1Pen1 correction (RewardLevel = 0, PenaltyLevel = 0)
        Rew1Pen1_error = find(strcmp(curSubData.CueDisplayed,'Rew1_Pen1') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 1);
        curSubData.PenaltyLevel(Rew1Pen1_error) = 0;
        curSubData.TaskBugCorrected(Rew1Pen1_error) = 1;
%         Rew1Pen1_total = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen1') & curSubData.RewardLevel == 0);
%         Rew1Pen1_correct = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen1') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 0);
%         Rew1Pen1_error = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen1') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 1);
%         if length(Rew1Pen1_total) == length(Rew1Pen1_correct) + length(Rew1Pen1_error)
%             curSubData.PenaltyLevel(Rew1Pen1_error) = 0;
%             curSubData.TaskBugCorrected(Rew1Pen1_error) = 1;
%         else
%             error(['Error! Rew1Pen1 TaskBugCorrected does not match for Subject ', num2str(subID)]);
%         end
        ...
        % lowRew: Rew1Pen2 correction (RewardLevel = 0, PenaltyLevel = 1)
        Rew1Pen2_error = find(strcmp(curSubData.CueDisplayed,'Rew1_Pen2') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 0);
        curSubData.PenaltyLevel(Rew1Pen2_error) = 1;
        curSubData.TaskBugCorrected(Rew1Pen2_error) = 1;
%         Rew1Pen2_total = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen2') & curSubData.RewardLevel == 0);
%         Rew1Pen2_correct = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen2') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 1);
%         Rew1Pen2_error = find(contains(curSubData.BlockType,'Rew') & strcmp(curSubData.CueDisplayed,'Rew1_Pen2') & curSubData.RewardLevel == 0 & curSubData.PenaltyLevel == 0);
%         if length(Rew1Pen2_total) == length(Rew1Pen2_correct) + length(Rew1Pen2_error)
%            curSubData.PenaltyLevel(Rew1Pen2_error) = 1;
%            curSubData.TaskBugCorrected(Rew1Pen2_error) = 1;
%         else
%             error(['Error! Rew1Pen2 TaskBugCorrected does not match for Subject ', num2str(subID)]);
%         end    
        
        %% COMPILE INTERVAL DATA TABLE
        % Create a table of the interval data that can be output for analyses (in R)
        % Can also use this table for analyses within matlab
        
        % First add NaNs if missing intervals in a given block
        if (numIntervalsTotal < numIntervals)
            curIntSubID = [curIntSubID, nan(1,numIntervals-length(curIntSubID))];
            curIntRewLvl = [curIntRewLvl, nan(1,numIntervals-length(curIntRewLvl))];
            curIntPenLvl = [curIntPenLvl, nan(1,numIntervals-length(curIntPenLvl))];
            curIntTotal = [curIntTotal, nan(1,numIntervals-length(curIntTotal))];
            curIntLength = [curIntLength(1:numIntervalsTotal), nan(1,numIntervals-numIntervalsTotal)]; 
            curIntLengthSecs = [curIntLengthSecs, nan(1,numIntervals-numIntervalsTotal)]; 
            curIntLengthSecsNoISI = [curIntLengthSecsNoISI,nan(1,numIntervals-numIntervalsTotal)];
            curIntCueDisplay = [curIntCueDisplay,num2cell(NaN(1,numIntervals-numIntervalsTotal))]; 
            curIntTaskBugCorrected = [curIntTaskBugCorrected, nan(1,numIntervals-numIntervalsTotal)];
            curIntNumErrorTrials = [curIntNumErrorTrials, nan(1,numIntervals-numIntervalsTotal)];
        end 
        
        curSubDataInt = table(curIntSubID',curIntRewLvl',curIntPenLvl',curIntCueDisplay',curIntTotal',curIntLength',curIntLengthSecs',curIntLengthSecsNoISI',...
            curIntTaskBugCorrected',curIntNumErrorTrials',curIntRewLvl',curIntPenLvl');
        curSubDataInt_header = {'SubID','RewardLevel','PenaltyLevel','CueDisplayed','Interval','IntervalLength_Ideal','IntervalLength_Actual','IntervalLengthNoISI_Actual',...
            'TaskBugCorrected','NumErrorTrials','RewardLevelTaskCode','PenaltyLevelTaskCode'};
        curSubDataInt.Properties.VariableNames = curSubDataInt_header;
        
        % CORRECT FOR BUG IN TASKCODE 
        % CueDisplayed not matching reward and penalty level for highrew
        % and lowrew blocks. however edit for all blocks just in case 
        
        % Rew2Pen1 correction
        Rew2Pen1_error = find(strcmp(curSubDataInt.CueDisplayed,'Rew2_Pen1') & curSubDataInt.RewardLevel == 1 & curSubDataInt.PenaltyLevel == 1);
        curSubDataInt.PenaltyLevel(Rew2Pen1_error) = 0;
        curSubDataInt.TaskBugCorrected(Rew2Pen1_error) = 1;
        
        % Rew2Pen2 correction
        Rew2Pen2_error = find(strcmp(curSubDataInt.CueDisplayed,'Rew2_Pen2') & curSubDataInt.RewardLevel == 1 & curSubDataInt.PenaltyLevel == 0);
        curSubDataInt.PenaltyLevel(Rew2Pen2_error) = 1;
        curSubDataInt.TaskBugCorrected(Rew2Pen2_error) = 1;
        
        % Rew1Pen2 correction
        Rew1Pen1_error = find(strcmp(curSubDataInt.CueDisplayed,'Rew1_Pen1') & curSubDataInt.RewardLevel == 0 & curSubDataInt.PenaltyLevel == 1);
        curSubDataInt.PenaltyLevel(Rew1Pen1_error) = 0;
        curSubDataInt.TaskBugCorrected(Rew1Pen1_error) = 1;
        
        % Rew1Pen1 correction
        Rew1Pen2_error = find(strcmp(curSubDataInt.CueDisplayed,'Rew1_Pen2') & curSubDataInt.RewardLevel == 0 & curSubDataInt.PenaltyLevel == 0);
        curSubDataInt.PenaltyLevel(Rew1Pen2_error) = 1;
        curSubDataInt.TaskBugCorrected(Rew1Pen2_error) = 1;      
...
pathToSave_Trial = strcat(pwd,'/DataFormatted/data_fMRI-TCB_RewardPenalty_Trial.csv');
writetable(subDataAll_Trial,pathToSave_Trial);
pathToSave_Interval = strcat(pwd,'/DataFormatted/data_fMRI-TCB_RewardPenalty_Interval.csv');
writetable(subDataAll_Interval,pathToSave_Interval);
...
%             curIntNum',curIntLength',curTrialsCompleted',curIntervalTrialNum',curRewLvl', curGainLvl',curStimCongruency',...
...
%         %curBlockNum = NaN(1,numTrialsTotal);
%         %curIntNum = NaN(1,numTrialsTotal);
%         curIntLength = NaN(1,numTrialsTotal);
%         %curTrialNuminInt = NaN(1,numTrialsTotal);
%         curTrialsCompleted = NaN(1,numTrialsTotal);
%         %curRewLvl = NaN(1,numTrialsTotal);
%         %curGainLvl = NaN(1,numTrialsTotal);
%         curIntLengthSecs = NaN(1,numTrialsTotal);
%         curIntLengthSecsNoISI = NaN(1,numTrialsTotal);
%         curBlockType = cell(1,numTrialsTotal);
%         curBlockType = cell(1,numTrialsTotal);
...
%         curIntBasedIntNum = NaN(1,numTrialsTotal);
%         curIntBasedLengthSecs = NaN(1,numTrialsTotal);
%         curIntBasedLengthSecsNoISI = NaN(1,numTrialsTotal); 
%         
%         
%         % Iterate over intervals to fill out arrays based on number of trials per interval
...
%             curIntervalTrialNum(ix_start:ix_end) = 1:numTrialsPerInterval(intNum);
...
% % NOTE: DY edid this, mostly renamed some variables and removed the initiation 
% % time related variables. Kept the previous code for historical purposes, may 
% % opt to delete if redundnant.
...
%             ];
%% Add Performance Variables
% allSubData.Acc(isnan(allSubData.Resp)) = nan; allSubData.AccRT = allSubData.Acc 
% .* allSubData.RT; allSubData.AccRT(allSubData.Acc==0) = nan; allSubData.logRT 
% = log(allSubData.RT); allSubData.logAccRT = log(allSubData.AccRT); allSubData.RespRateAll(isnan(allSubData.RT)) 
% = nan;
...
% % Assuming congruency is 3-level (incong =0, cong=1, neutral=2): allSubHdr 
% = {'SubID','Vers','BlockNum', 'BlockType', 'IntervalNum','IntervalLength','TrialsInInterval',... 
% 'RewLvl', 'GainLvl','Congruency','InitiationRT','RT','CorrectResp','Resp','Acc',... 
% 'RespRateAll','RespRateCorr','IntLength','IntTrialNum','IsComplete',... 'BlockTypeRlvl','BlockTypeGlvl','IntLengthCORRECTED','RespRateCorrCORRECTED'}; 
